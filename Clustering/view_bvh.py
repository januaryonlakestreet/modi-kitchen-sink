from Clustering.BvHparser import Bvh
import numpy as np
...
def ProcessBVH(filename):
    ...
    return [joints, joints_offsets, joints_hierarchy, root_positions, joints_rotations, joints_saved_angles,
            joints_positions, joints_saved_positions]
...
def _calculate_frame_joint_positions_in_local_space(joints, joints_offsets, frame_joints_rotations,
                                                    joints_saved_angles,
                                                    joints_hierarchy):
    local_positions = {}
    ...
    for joint in joints:
        ...
        for i, con_joint in enumerate(connected_joints):
            if i == 0:
                ...
                Rot = Rot @ _get_rotation_chain(joints_saved_angles[parent_joint],
...
def _calculate_frame_joint_positions_in_world_space(local_positions, root_position, root_rotation,
                                                    saved_angles):
    world_pos = {}
...
class BvhViewer:
    ...
    def __init__(self,location,seed):
        self.Seed = seed
        self.Location = location


    def DrawBVH(self):
        skeleton_data = ProcessBVH(self.Location)
        ...
        joints_saved_angles = skeleton_data[
            5]  # this contains channel information. E.g ['Xrotation', 'Yrotation', 'Zrotation']
        joints_positions = skeleton_data[6]
        joints_saved_positions = skeleton_data[7]

        self._Draw_bvh(joints, joints_offsets, joints_hierarchy, root_positions, joints_rotations, joints_saved_angles)

    def _Draw_bvh(self, joints, joints_offsets, joints_hierarchy, root_positions, joints_rotations, joints_saved_angles):
        fig = plt.figure()
        ...
        while True:
            for i in range(0, len(joints_rotations), frame_skips):
                ...
                local_pos = _calculate_frame_joint_positions_in_local_space(joints, joints_offsets,
                                                                            frame_joints_rotations,
                                                                            joints_saved_angles, joints_hierarchy)

                # calculate world positions
                world_pos = _calculate_frame_joint_positions_in_world_space(local_pos, root_positions[i],
                                                                            frame_joints_rotations[joints[0]],
                                                                            joints_saved_angles[joints[0]])
                ...
                ax.set_title('seed value : + ' + str(self.Seed) + ' frame: ' + str(i))
                plt.pause(0.001)
                ax.cla()
