...
def calc_attribute_score(attr, data_path, score_path, **kwargs):
    ...
    for feature, val in zip(to_keep, [score_all[:, np.newaxis], W_all,
                                      seeds]):  # scores_all should be of size [n,1] (required by train_boundary)
...
def calc_angle(anim, names, center_joint_name, end_joint_name, kwargs):
    ...
    assert (np.inner(offset_to_end_normed, offset_to_end_normed) == np.sum(
        offset_to_end_normed * offset_to_end_normed)).all()
...
def r_wrist_accel_score(anim, names, joint_locations, bone_lengths, **kwargs):
    r_wrist_loc_relative_to_r_elbow = joint_locations[names.index(kwargs['r_wrist'])] - joint_locations[
        names.index(kwargs['r_elbow'])]
    r_wrist_accel = r_wrist_loc_relative_to_r_elbow[:, :-2] - 2 * r_wrist_loc_relative_to_r_elbow[:,
                                                                  1:-1] + r_wrist_loc_relative_to_r_elbow[:, 2:]
...
def latent_space_edit(args, model, entity, attributes, stages):
    ...
    for attribute in attributes:
        ...
        # stage edit
        edit(args=args, boundary_path=boundary_path, extreme_idx=extreme_idx, model=model, seeds=seeds,
             score_path=score_path,
             stages=stages, entity=entity)

    pd.Series({'type': 'multi_edits', 'entity': entity}).to_csv(osp.join(data_path, 'args.csv'), sep='\t',
...
def calc_boundary(latent_codes, score_path, scores, stages):
    ...
    if stages['train_boundary'] is None:
        ...
        boundary_normal, boundary_const = train_boundary(latent_codes=latent_codes, scores=scores,
                                                         chosen_num_or_ratio=ratio)
        ...
        boundary_normal, boundary_const = np.load(osp.join(boundary_path, 'boundary.npy'), allow_pickle=True)[
            0].values()
