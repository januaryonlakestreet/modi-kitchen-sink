#for testing the models found in /evaluation/models/stgcn.py
...
def generate(args, g_ema, device, mean_joints, std_joints, entity):
    ...
    generated_seeds = generated_motions.index.values
    ...
    return generated_motions,generated_seeds
...
def initialize_model(device, modelpath, dataset='humanact12'):
    if dataset == 'mixamo':
        num_classes = 15
...
def main(args_not_parsed):
    ...
    if args.test_model:
        # generate motions
        generated_motions,generated_seeds = generate(args, g_ema, device, mean_joints, std_joints, entity=entity)
    ...
    iterator_generated = DataLoader(generated_motions, batch_size=64, shuffle=False, num_workers=6)
    ...
    iterator_dataset = DataLoader(motion_data, batch_size=64, shuffle=False, num_workers=6)
    ...
    classid = 0
    SeedsToPlot = []
    results = list(zip(generated_motions,generated_predictions.max(dim=1).indices,generated_seeds))
