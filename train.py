    ...
    for idx in pbar:
        ...
        if i >= 2000 and i % 2000 == 0 and args.action_recog_model is not None:
         #   fid, kid, g_diversity = calc_evaluation_metrics(args, device, g_ema, entity, std_joints, mean_joints)
            loss_dict['evaluation_metrics_fid'] = 0
            loss_dict['evaluation_metrics_kid'] = 0
            loss_dict['evaluation_metrics_g_diversity'] = 0
...
def main(args_not_parsed):
    ...
    mode = 0o777
    os.makedirs(images_output_folder, exist_ok=True,mode=mode)
    os.makedirs(animations_output_folder, exist_ok=True,mode=mode)
    os.makedirs(args.model_save_path, exist_ok=True,mode=mode)
